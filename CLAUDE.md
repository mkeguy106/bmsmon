...
reading at a time into a `MotionGate`, **asymmetrically**:

- **null or stale** — older than `MOTION_STALE_MS = 150_000` — → **fails open**, gate reset.
- **confidence < `STILL_CONFIDENCE_MIN` (75)** → **holds `prev` unchanged**. Uncertainty neither
  pauses nor resumes; the last confident verdict stands.
- **confident `STILL`** → run++, gate closes once the run reaches `STILL_DEBOUNCE_N = 3`.
...
**Branch order is load-bearing: staleness is checked *before* uncertainty**, so a dead signal can
never hold the gate shut. Every unusable-signal path — permission denied, AR unavailable on the
device, subscription lapsed, process restarted with no reading yet, updates gone stale — fails open
to GPS-on. That is the user's explicit choice: never lose an outing, even at the cost of the saving.
The single-sample `confidentlyStill()` predicate is **deleted**; the name survives only as
...
`MotionSource` straight off `gpsWanted`, and folds each reading **inside the same `@Synchronized
applyGpsGate`** that writes `gpsActive` (same lock discipline as `locationSource`, so a `start()`
can't land after a concurrent teardown's `stop()`); `shutdownGps()` stops the source and **resets the
gate**, so a stale debounce run cannot survive a stop. Two build pitfalls, both **silent** failures
rather than crashes: the AR `PendingIntent` must be `FLAG_MUTABLE` (Play Services fills the
`ActivityRecognitionResult` extra into it) **and** explicit (`Intent(ACTION).setPackage(packageName)`)
— Android 14+ throws `IllegalArgumentException` for mutable + implicit, so motion sensing would
simply never have subscribed; and `ACTIVITY_RECOGNITION` must be requested at **both** `ui/App.kt`
call sites, mirroring `askNotificationPermission()`, because on any install where BLE is already
granted — the real device and every existing user — the monitor toggle takes the `hasBlePermissions`
branch and the `permLauncher` site never fires. `Settings › Battery saver` carries a **read-only**
...
`MOTION_STALE_MS` (150 s) trips staleness and reopens the gate. Measured on-device 2026-08-07: the
debounce math itself is exact to sub-second precision (every close tied to a 3-reading confident-STILL
burst, every reopen to staleness firing at 150.067 s / 150.170 s after the last reading, and **zero**
instances of the old reopen-on-`UNKNOWN` bug), and the gate held closed for one unbroken **5m18s**
stretch — then cycled open→closed→open→closed→open over ~16 min and sat open the final 7+ min with no
readings at all (no crash, no subscription error, device awake, not in Doze). Delivery sparsity is a
**different problem** from the flapping the hysteresis fixed, and it bounds the achievable saving.
...
that has read zero since 2026-08-03.
