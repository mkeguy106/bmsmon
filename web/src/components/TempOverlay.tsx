import { formatDelta, marginToCutoffC, zoneLabel, type TempUnit } from "../temp";
 ...
 */
export function TempOverlay({ worst, unit, onAck }:
  { worst: PackTemp; unit: TempUnit; onAck: () => void }) {
  const label = zoneLabel(worst.zone.key);
  const detail = worst.zone.rank >= 4
    ? `${label} · ${worst.name} · LOAD OFFLINE`
    : `${label} · ${worst.name} · ${formatDelta(Math.max(0, Math.round(marginToCutoffC(worst.tempC, worst.zone.side))), unit)} TO CUTOFF`;
  return (
